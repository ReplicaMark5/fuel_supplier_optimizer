...
if [[ ! -f "$src" ]]; then
  ...
  {
    echo "<!-- Edit ONLY this file (CONTEXT.md). Targets like CLAUDE.md / AGENTS.md / GEMINI.md are read only and generated. (after edits to the CONTEXT.MD run .context-sync/manual_sync_context.sh to make the changes sync across to the target files) -->"
    echo
...
for t in "${targets[@]}"; do
  ...
  sum_t="$( [ -f "$target_file" ] && sha256sum "$target_file" | awk '{print $1}' || echo "" )"
...
sed -i "s/__TARGETS__/(${targets_str})/" "$sync_dir/manual_sync_context.sh"
...
if $legacy_found && [ "$force" = false ] && [ "$auto" = false ]; then
  ...
  # Run sync from the sync directory
